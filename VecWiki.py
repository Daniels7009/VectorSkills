...
def get_picture(page):
    ...
    for pic in page.images:
        if ".jpg" in pic or ".png" in pic or ".jpeg" in pic:
            pics.append(pic)
            #print(pic)
    # and select one to display
    num_pics = len(pics)-1
    if num_pics > 0:    
        pic = pics[r.randint(0, num_pics)]
    elif num_pics == 0:
        pic = pics[0]
    else:
        pic = ("https://upload.wikimedia.org/wikipedia/commons/5/53/Wikipedia-logo-en-big.png")
